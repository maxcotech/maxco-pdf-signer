...
PdfSigner (src/PdfSigner.ts)          ← public API, enforces phase order
 ...
 │   ├─ coordinates.ts                 ← origin/units → PDF user space
 │   └─ VisualStamper.types.ts
 ├─ inspectPdf (src/engine/)           ← Phase 0: read-only geometry (no mutation)
...
client/index.ts        ← typed HTTP client, published as `pdf-signer/client`.
                         Depends on nothing in src/ — it talks to the REST API
                         over fetch and duplicates the types deliberately, so it
                         stays usable in a browser and in other repos.
```
...
VisualStamper.applyStamp()
    ...
    • PDFDocument.load() → read page geometry (size + normalised /Rotate)
    • resolveStampPosition(): declared origin/units → PDF user space  ← §5.2a
    • rasteriseSvgToPng(): Resvg renders SVG → sharp resizes + flattens alpha → PNG
      (sized from the RESOLVED points, not the caller's pixels)
    • embedPng() → page.drawImage() at the resolved rectangle
...
PdfEngine.injectSignature()
    ...
    │
    ▼
SignedPdfResult { signedPdf, documentHash, pkcs7Hex, byteRange, signingTime, stampRect? }
```

**Phase 0 (optional, read-only)**: `inspectPdf()` reports page geometry so a caller can build a `StampPosition` at all. It never mutates the document and is outside the ordering constraint — call it whenever.
...
│   │   ├── inspectPdf.ts             read-only page geometry / encryption / signature count
│   │   └── PdfEngine.types.ts        PreparedPdf, ByteRange, SignaturePlaceholderOptions,
│   │                                 PdfInspection, PdfPageInfo
│   ├── utils/
...
│       ├── coordinates.ts            resolveStampPosition, normaliseRotation, canvasYToPdfY
│       ├── VisualStamper.types.ts    SignatureAppearance, StampPosition, ResolvedStampPosition,
│       │                             PageGeometry, VisualStampResult
...
├── client/
│   └── index.ts                     typed REST client → `pdf-signer/client` (dist-client/)
├── INTEGRATION.md                   consumer-facing guide for apps CALLING the service
├── test/
...
├── docs/openapi.json                committed OpenAPI 3.1 contract (npm run openapi:emit)
├── scripts/
...
| `inspect(buffer)` | Read-only page geometry + encryption/signature status. Delegates to `inspectPdf` |
| `verifyByteRangeIntegrity(buffer)` | Arithmetic consistency check only (not crypto verify) |

`signLocal` and `signRemote` share the same phase-1 stamping code path. The difference begins at `CryptoStore`. Both attach `stampRect` (the resolved rectangle from phase 1) to the `SignedPdfResult` when an appearance was supplied — `CryptoStore` never sees the stamp, so the facade is the only place that can report it.
...
Validates appearance → resolves SVG → loads PDF → reads page geometry → resolves the rectangle → rasterises → embeds PNG → draws → saves. Returns `VisualStampResult` with `stampedPdfBuffer`, `resolvedPosition` and `pageGeometry`.
...
- Default `renderScale: 2` renders the PNG at 2× the **resolved** stamp point dimensions for crispness
- PDF saved with `{ useObjectStreams: false }` — mandatory (see §7.1)
- Page index is 0-based; validated against `pdfDoc.getPageCount()`

**Order note**: the PDF is loaded *before* the rectangle is resolved, because conversion from a browser space needs the real page height and `/Rotate`. Rasterisation happens after resolution so a pixel-measured stamp is rendered at the right resolution rather than at whatever the caller's viewport happened to be. Don't reorder these — rasterising first would make the PNG size depend on the client's zoom level.

**`canvasYToPdfY(canvasY, stampHeightPts, pageHeightPts, pixelsPerPoint?)`** — static utility, delegates to `coordinates.ts`

Retained for callers doing their own arithmetic. Prefer `position.origin` / `position.units`, which resolve against the page geometry actually read from the document.

---

### 5.2a `src/visual/coordinates.ts`

Maps a caller-supplied stamp rectangle into PDF user space. This is the single most common integration bug, and it is silent — a wrong conversion produces a cryptographically perfect signature in the wrong place — so it lives in one place, runs on every stamp, and refuses ambiguous input rather than guessing.

**`resolveStampPosition(position, pageGeometry) → ResolvedStampPosition`**

| Declared space | Behavior |
|---|---|
| `units: 'pt'`, `origin: 'bottom-left'` (defaults) | Pass-through, bounds-checked only |
| `origin: 'top-left'` | `y = pageHeightPts - y - height` |
| `units: 'px'` | Scale by `pageWidthPts / viewportWidth` (or the height equivalent) |

Rejections, all `InvalidPositionError`:
- `width` or `height` ≤ 0
- `units: 'px'` with neither `viewportWidth` nor `viewportHeight`
- viewport fields supplied with `units: 'pt'` — a sign the caller meant `'px'`
- both viewport dimensions given but implying scales that differ by more than `SCALE_AGREEMENT_TOLERANCE` (2%) — means the caller measured a container, not the rendered page
- any conversion requested for a page with non-zero `/Rotate`. A quarter-turned page displays a different box than the space it draws in, and a correct mapping needs a full affine transform; refusing beats placing the stamp somewhere plausible but wrong. User-space points still pass through, since `/Rotate` does not affect that space.
- a resolved rectangle with **no** intersection with the page. Overhanging one edge is legitimate bleed; missing entirely draws nothing and is the classic unconverted browser coordinate.

`normaliseRotation(angle)` folds any `/Rotate` value to 0/90/180/270.

**If you add a new declared space**, add it here and nowhere else — `VisualStamper` calls this once and both signing paths inherit it. Then mirror the field on the server's `positionShape` (§11.9) and on `client/index.ts`'s `StampPosition`.

---

### 5.2b `src/engine/inspectPdf.ts`

**`inspectPdf(pdfBuffer) → PdfInspection`**

Read-only. Reports `pageCount`, per-page `widthPts`/`heightPts`/`rotation`/`displayWidthPts`/`displayHeightPts`, plus `encrypted`, `signatureCount` and `signable`.

This exists because a `StampPosition` is meaningless without the page dimensions it is relative to. Without it every integrator re-derives the geometry with their own PDF parser.

Two deliberate choices:
- **Encryption is reported, not thrown.** `PDFDocument.load` is called with `ignoreEncryption: false` and an `EncryptedPDFError` becomes `encrypted: true` with an empty `pages` array. The caller needs the fact in order to explain the rejection; only a genuinely unreadable buffer throws `InvalidPdfError`.
- **Signatures are counted by scanning raw `latin1` bytes for `/ByteRange [`**, not by walking the object graph. A signature in an earlier incremental revision may not surface as the current `/AcroForm`, and the question is about the whole file. `latin1` keeps the byte-to-char mapping 1:1 so stream data cannot shift offsets.

`displayWidthPts`/`displayHeightPts` swap on a quarter turn: a pixel rectangle measured over a *rendered* page is relative to the display box, while stamp coordinates are in the MediaBox. Both are reported so a caller can tell them apart.
...
### `StampPosition` (PDF points, bottom-left origin — by default)
...
interface StampPosition {
  ...
  // Declared space. Resolved by src/visual/coordinates.ts (§5.2a).
  origin?: 'bottom-left' | 'top-left';   // default 'bottom-left'
  units?: 'pt' | 'px';                   // default 'pt'
  viewportWidth?: number;                // px only: rendered page width in px
  viewportHeight?: number;               // px only: interchangeable with the above
}
```

Page sizes in points: A4 = 595.28 × 841.89 | Letter = 612 × 792 | Legal = 612 × 1008 — or call `inspect()` for the real ones.

The defaults preserve the original PDF-native contract, so existing callers are unaffected. The optional fields exist because callers overwhelmingly obtain the rectangle from a browser, where all three of origin, unit and scale differ.
...
interface PdfInspection {
  ...
  encrypted: boolean;       // cannot be signed — reject the upload
  signatureCount: number;   // > 0 means already signed (counter-signing is legal)
  signable: boolean;
}

interface PdfPageInfo {
  index: number;             // 0-based; the value StampPosition.page takes
  widthPts: number;          // user space (MediaBox) — what stamp coords use
  heightPts: number;
  rotation: number;          // 0 | 90 | 180 | 270
  displayWidthPts: number;   // what a viewer shows; swaps on a quarter turn
...
interface SignedPdfResult {
  ...
  stampRect?: ResolvedStampPosition;  // where the stamp landed, in points,
                                      // bottom-left origin. Absent with no appearance.
}
```
...
| `test/coordinates.test.ts` | `resolveStampPosition` arithmetic, every rejection, and that the resolved rect is what reaches the draw call |
| `test/inspect.test.ts` | `inspectPdf` + the HTTP route; geometry feeding straight into a working sign call |
| `test/client.test.ts` | `PdfSignerClient` against a real listening server — not a mocked fetch |
| `test/openapi.test.ts` | Route/spec parity, error-code parity, committed spec freshness |
| `test/p12-encoding.test.ts` | Raw-DER vs base64 P12 detection |

**Coordinate tests assert exact arithmetic, not "did not throw."** A wrong conversion still produces a valid signature — the failure mode is a stamp in the wrong place, which no smoke test catches.

**The client is tested over a socket on purpose.** Its whole reason for existing is that hand-built multipart bodies for this endpoint are easy to get subtly wrong, so a test that stubs the transport would validate nothing that matters.
...
Three routes today: `GET /health`, `POST /api/v1/documents/inspect` (read-only geometry) and `POST /api/v1/sign`. The intended client flow is inspect → place → sign, because a stamp rectangle cannot be built without the page dimensions inspect returns.
...
7. Add a method to `client/index.ts` (§11.10) and a case to `test/client.test.ts`
8. Document it in `INTEGRATION.md` — the consumer-facing guide, which no test enforces
...
**Validate in the schema when the rule allows it.** A cross-field rule that depends only on the request (`units: 'px'` needs a viewport; viewport fields need `units: 'px'`) belongs in a `superRefine` on the Zod schema, so it returns `VALIDATION_ERROR` with a `details` entry naming the field. Rules that depend on the *document* — page rotation, off-page rectangles, page index — can only be checked after parsing and surface as the library's `INVALID_POSITION`. Follow the existing split: register the plain shape as the OpenAPI component, and wire the refined version into `jsonFormFields`.

### 11.10 Extending the typed client

`client/index.ts` is published as `pdf-signer/client` and compiled separately (`tsconfig.client.json` → `dist-client/`, `lib: ["ES2022", "DOM"]`).

It **duplicates the request/response types rather than importing from `src/`**, and that is deliberate: it must run in a browser and in other repos without pulling in pdf-lib, sharp or resvg. The cost is that a contract change has to be mirrored here by hand — which is why the client is covered by a test that talks to a real server rather than a mock.

Rules when touching it:
1. Keep it dependency-free — global `fetch`/`FormData`/`Blob` only
2. Every new request field that is a JSON form part must go through `JSON.stringify` in `sign()`, never appended as an object
3. Map new response headers into the returned object; do not make callers read headers
4. New error codes go in the `ErrorCode` union
5. Add the case to `test/client.test.ts` and the field to `INTEGRATION.md`

`toBlob()` copies input bytes into a fresh `Uint8Array` before wrapping: a Node `Buffer` is a view onto a shared pool, so passing one to `Blob` directly can capture unrelated bytes.
...
**Symptom**: Stamp appears at the wrong position. Since a rectangle that still lands on the page throws nothing, this is caught by eye or not at all.
**Cause**: Browser coordinates (top-left origin, pixels of a rendered page) passed as if they were PDF points from the bottom edge.
**Fix**: Declare the space instead of converting — `origin: 'top-left'`, `units: 'px'`, `viewportWidth: <rendered page width in px>`. `resolveStampPosition` (§5.2a) then converts against the page geometry read from the document, and `result.stampRect` reports what it used. A rectangle that misses the page entirely is now rejected with `InvalidPositionError` rather than silently drawing nothing.

### P11: Assuming `widthPts` is what the user sees

**Symptom**: A stamp on a rotated page lands on the wrong axis, or a px conversion is rejected for a page that renders fine in a viewer.
**Cause**: `/Rotate` makes the displayed box differ from the MediaBox. `inspect()` reports `widthPts`/`heightPts` (user space, what stamp coordinates use) *and* `displayWidthPts`/`displayHeightPts` (what a viewer shows) for exactly this reason.
**Fix**: Do stamp arithmetic against `widthPts`/`heightPts`. For a page with non-zero `rotation`, send user-space points — px/top-left conversion is deliberately refused there rather than approximated.

### P12: Documenting a JSON form field as an object

**Symptom**: A generated client sends a nested multipart part and every request fails validation.
**Cause**: `metadata`, `appearance` and `position` are form fields carrying **JSON strings**. Documenting them as objects describes a wire format the server does not accept.
**Fix**: Use the `jsonEncoded()` helper in `server/schemas/sign.ts` (§11.9). In a consumer, `JSON.stringify` each one — or use `pdf-signer/client`, which exists largely to make this unmissable.
...
New coordinate     → visual/coordinates.ts ONLY → server/schemas/sign.ts positionShape
  space              → client/index.ts StampPosition → INTEGRATION.md §5
...
New error          → errors.ts → index.ts → docs/API.md → INTEGRATION.md §8 table
New HTTP endpoint  → server/schemas/ → server/routes/ → server/openapi/routes.ts
                     → npm run openapi:emit → server/app.ts → client/index.ts
New test           → test/[suite].test.ts, assert ByteRange + documentHash independently
Build              → npm run build  (lib → dist/, client → dist-client/)
...
**Anything that changes the HTTP contract must reach four places**: the Zod schema (validation + spec), the committed `docs/openapi.json`, `client/index.ts`, and `INTEGRATION.md`. The first two are enforced by `test/openapi.test.ts`; the last two are not, so they are the ones that rot. `INTEGRATION.md` is the document a consuming app's agent reads — CLAUDE.md is for agents changing pdf-signer itself, and pointing an integrator at it is a mistake.
